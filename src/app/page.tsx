...
export default function Home() {
  ...
  // ADMIN 环境变量指定账号 + 管理后台会话角色（admin / superadmin）。
  // 普通用户（即使是 Pro）在 DOM 中完全看不到“控制面板”入口。
  ...
  if (adminSession && !adminSession.pending) {
    return <AdminDashboardPanel session={adminSession} onLogout={handleAdminLogout} />;
  }
  ...
  return (
    <div className="app">
      ...
      <Header
        ...
        onLogoDoubleClick={
          isAdminIdentity ? () => setAdminSession({ pending: true }) : undefined
        }
      />

      {/* Request admin pending if double-click triggered */}
      {adminSession?.pending && <AdminLoginPanel onLogin={handleAdminLogin} />}
      ...
      <footer className="footer">
        <div className="version-bar">
          ...
          {isAdminIdentity && (
            <a className="admin-entry" href="/admin">{t("admin_entry")}</a>
          )}
        </div>
      </footer>
